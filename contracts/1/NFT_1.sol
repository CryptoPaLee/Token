    ...
    DefaultOperatorFilterer
{
    ...
    function mintPresale(uint qty) external payable noContract nonReentrant {
        require(state == State.Presale, "NO_SALE");
        require(msg.value * qty >= mintPrice, "INSUFFICIENT_ETHER");
        require(totalSupply() + qty <= presaleSupply, "NO_MORE_SUPPLY");
        require(
            mintedPerWallet[msg.sender] + qty <= maxPerWallet,
            "MAX_PER_WALLET"
        );
